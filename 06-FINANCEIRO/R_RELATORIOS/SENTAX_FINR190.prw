    ...
    If lRelMulNat
        ...
        While nI <= Len(aRelat)
            ...
            If !(Empty(aRelat[nI][ID_E5RECNO]))
                SE5->(MsGoTo(aRelat[nI][ID_E5RECNO]))
            EndIf
...
/*/{Protheus.doc} InitReport
Efetua a inicialização do relatório.

@type       function
@author     Rafael Riego
@since      25/09/2020
@param      oReport, object, objeto do relatório
@param      aRelat, array, array contendo os dados a serem setados no bloco de código
@param      nI, numeric, contador a ser utilizado dentro do relatório (com o array aRelat)
@param      cTotText, character, variável texto do totalizador
@return     Nil
/*/
...
/*/{Protheus.doc} PrintTot
Imprime os totais "Baixados", "Mov Fin.", "Compens."

@type       function
@author     Daniel Tadashi Batori
@since      10/10/2006
@param      aTotais, array, array a ser utilizado para impressão
@param      oReport, object, objeto do relatório
@param      lFil, logical, .F. se for total da seção ou .T. se for da filial
@param      nBreak, numeric, tipo da quebra
@param      nJ, numeric, contador
@return     Nil
/*/
...
Static Function FA190ImpR4(aRet As Array, nOrdem As Numeric, aTotais As Array, oReport As Object, nGerOrig As Numeric, nI As Numeric, cTotText As Character) As Array
    ...
    Local cInAbatim     As Character
    Local cInPagAnt     As Character
    Local cInRecAnt     As Character
    ...
    Local cUpdate3      As Character
    Local cUpdate4      As Character
    Local cUpdate5      As Character
    Local cUpd5Fil      As Character
    Local cDlt2Fil      As Character
    ...
    If MV_PAR16 == 1
        cUpdate3 := "UPDATE " + cTable + " SET SE5MANUAL = 1 " 
        cUpdate3 += "WHERE (E5_TIPODOC = ' ' OR E5_NUMERO = ' ') AND D_E_L_E_T_ = ' ' "
        
        If TCSqlExec(cUpdate3) < 0
            DbOnError(TCSqlError(), "cUpdate3")
            Return Nil
        EndIf
    EndIf

    cInPagAnt := FormatIn(MVPAGANT + "|" + MV_CPNEG, "|") 
    cInRecAnt := FormatIn(MVRECANT + "|" + MV_CRNEG, "|")
    cInAbatim := FormatIn(MVABATIM, "|")

    cUpdate4 := "UPDATE " + cTable + " SET SE5CARTEIR = 'R' "
    cUpdate4 += "WHERE D_E_L_E_T_ = ' ' "
    cUpdate4 += "AND ((E5_RECPAG = 'R' AND E5_TIPO NOT IN " + cInPagAnt + ") "
    cUpdate4 += "OR (E5_RECPAG = 'P' AND E5_TIPO IN " + cInRecAnt + ") "
    cUpdate4 += "OR (E5_RECPAG = 'P' AND E5_TIPODOC = 'E2')) "
    
    If TCSqlExec(cUpdate4) < 0
        DbOnError(TCSqlError(), "cUpdate4")
        Return Nil
    EndIf
    
    If cPaisLoc == "BRA"
        If lOracle
            cUpdate5 := "UPDATE " + cTable + " SET " + cTable + ".SE5VERABAT = 'S' WHERE "
            cUpdate5 += cTable + ".E5_FILIAL = '?2' "
            cUpdate5 += "AND " + cTable + ".SE5MANUAL = 0 "
            cUpdate5 += "AND " + cTable + ".SE5MAXSEQ = " + cTable + ".E5_SEQ "
            cUpdate5 += "AND " + cTable + ".D_E_L_E_T_ = ' ' "
            
            If mv_par11 == 1
                cUpdate5 += "AND " + cTable + ".SE5CARTEIR = 'R' "
                cUpdate5 += "AND EXISTS ("
                cUpdate5 += "SELECT SE1.E1_FILIAL FROM " + RetSQLName("SE1") + " SE1 "
                cUpdate5 += "WHERE SE1.E1_FILIAL = '?1' "
                cUpdate5 += "AND SE1.E1_PREFIXO = " + cTable + ".E5_PREFIXO "
                cUpdate5 += "AND SE1.E1_NUM = " + cTable + ".E5_NUMERO "
                cUpdate5 += "AND SE1.E1_PARCELA = " + cTable + ".E5_PARCELA "
                cUpdate5 += "AND SE1.E1_TIPO = " + cTable + ".E5_TIPO "
                cUpdate5 += "AND SE1.E1_CLIENTE = " + cTable + ".E5_CLIFOR "
                cUpdate5 += "AND SE1.E1_LOJA = " + cTable + ".E5_LOJA "
                cUpdate5 += "AND SE1.E1_SALDO = 0 AND SE1.D_E_L_E_T_ = ' ' "
                cUpdate5 += "AND EXISTS ("
                cUpdate5 += "SELECT E1_TIPO FROM " + RetSQLName("SE1") + " SE1TIT "
                cUpdate5 += "WHERE SE1TIT.E1_FILIAL = SE1.E1_FILIAL "
                cUpdate5 += "AND SE1TIT.E1_TITPAI = SE1.E1_PREFIXO||SE1.E1_NUM||SE1.E1_PARCELA||SE1.E1_TIPO||SE1.E1_CLIENTE||SE1.E1_LOJA "
                cUpdate5 += "AND SE1TIT.D_E_L_E_T_ = ' ' "
                cUpdate5 += "AND SE1TIT.E1_TIPO IN " + cInAbatim + "))"
            Else
                cUpdate5 += "AND " + cTable + ".SE5CARTEIR = 'P' "
                cUpdate5 += "AND EXISTS ("
                cUpdate5 += "SELECT SE2.E2_FILIAL FROM " + RetSQLName("SE2") + " SE2 "
                cUpdate5 += "WHERE SE2.E2_FILIAL = '?1' "
                cUpdate5 += "AND SE2.E2_PREFIXO = " + cTable + ".E5_PREFIXO "
                cUpdate5 += "AND SE2.E2_NUM = " + cTable + ".E5_NUMERO "
                cUpdate5 += "AND SE2.E2_PARCELA = " + cTable + ".E5_PARCELA "
                cUpdate5 += "AND SE2.E2_TIPO = " + cTable + ".E5_TIPO "
                cUpdate5 += "AND SE2.E2_FORNECE = " + cTable + ".E5_CLIFOR "
                cUpdate5 += "AND SE2.E2_LOJA = " + cTable + ".E5_LOJA "
                cUpdate5 += "AND SE2.E2_SALDO = 0 AND SE2.D_E_L_E_T_ = ' ' "
                cUpdate5 += "AND EXISTS ("
                cUpdate5 += "SELECT E2_TIPO FROM " + RetSQLName("SE2") + " SE2TIT "
                cUpdate5 += "WHERE SE2TIT.E2_FILIAL = SE2.E2_FILIAL "
                cUpdate5 += "AND SE2TIT.E2_TITPAI = SE2.E2_PREFIXO||SE2.E2_NUM||SE2.E2_PARCELA||SE2.E2_TIPO||SE2.E2_FORNECE||SE2.E2_LOJA "
                cUpdate5 += "AND SE2TIT.D_E_L_E_T_ = ' ' "
                cUpdate5 += "AND SE2TIT.E2_TIPO IN " + cInAbatim + "))"            
            EndIf
        Else        
            cUpdate5 := "UPDATE " + cTable + " SET SE5VERABAT = 'S' FROM "
            
            If mv_par11 == 1
                cUpdate5 += RetSQLName("SE1") + " SE1 WHERE "
                cUpdate5 += "SE1.E1_FILIAL = '?1' " 
                cUpdate5 += "AND SE1.E1_PREFIXO = " + cTable + ".E5_PREFIXO "
                cUpdate5 += "AND SE1.E1_NUM = " + cTable + ".E5_NUMERO "
                cUpdate5 += "AND SE1.E1_PARCELA = " + cTable + ".E5_PARCELA "
                cUpdate5 += "AND SE1.E1_TIPO = " + cTable + ".E5_TIPO "        
                cUpdate5 += "AND SE1.E1_CLIENTE = " + cTable + ".E5_CLIFOR "
                cUpdate5 += "AND SE1.E1_LOJA = " + cTable + ".E5_LOJA "
                cUpdate5 += "AND SE1.E1_SALDO = 0 "
                cUpdate5 += "AND SE1.D_E_L_E_T_ = ' ' " 
                cUpdate5 += "AND EXISTS (
                cUpdate5 += "SELECT E1_TIPO FROM " + RetSQLName("SE1") + " SE1TIT "
                cUpdate5 += "WHERE SE1TIT.E1_FILIAL = SE1.E1_FILIAL " 
                
                If cSGBD $ "MSSQL7"
                    cUpdate5 += "AND SE1TIT.E1_TITPAI = SE1.E1_PREFIXO" + cOperador + "SE1.E1_NUM" + cOperador + "SE1.E1_PARCELA"
                    cUpdate5 += cOperador + "SE1.E1_TIPO" + cOperador + "SE1.E1_CLIENTE" + cOperador + "SE1.E1_LOJA "
                Else
                    cUpdate5 += "AND SE1TIT.E1_TITPAI = CONCAT(SE1.E1_PREFIXO,SE1.E1_NUM,SE1.E1_PARCELA,SE1.E1_TIPO,SE1.E1_CLIENTE,SE1.E1_LOJA) "
                EndIf
                
                cUpdate5 += "AND SE1TIT.D_E_L_E_T_ = ' ' "
                cUpdate5 += "AND SE1TIT.E1_TIPO IN " + cInAbatim + ") "
            Else
                cUpdate5 += RetSQLName("SE2") + " SE2 WHERE "
                cUpdate5 += "SE2.E2_FILIAL = '?1' " 
                cUpdate5 += "AND SE2.E2_PREFIXO = " + cTable + ".E5_PREFIXO "
                cUpdate5 += "AND SE2.E2_NUM = " + cTable + ".E5_NUMERO "
                cUpdate5 += "AND SE2.E2_PARCELA = " + cTable + ".E5_PARCELA "
                cUpdate5 += "AND SE2.E2_TIPO = " + cTable + ".E5_TIPO "        
                cUpdate5 += "AND SE2.E2_FORNECE = " + cTable + ".E5_CLIFOR "
                cUpdate5 += "AND SE2.E2_LOJA = " + cTable + ".E5_LOJA "
                cUpdate5 += "AND SE2.E2_SALDO = 0 "
                cUpdate5 += "AND SE2.D_E_L_E_T_ = ' ' " 
                cUpdate5 += "AND EXISTS (
                cUpdate5 += "SELECT E2_TIPO FROM " + RetSQLName("SE2") + " SE2TIT "
                cUpdate5 += "WHERE SE2TIT.E2_FILIAL = SE2.E2_FILIAL " 
                
                If cSGBD $ "MSSQL7"
                    cUpdate5 += "AND SE2TIT.E2_TITPAI = SE2.E2_PREFIXO" + cOperador + "SE2.E2_NUM" + cOperador + "SE2.E2_PARCELA"
                    cUpdate5 += cOperador + "SE2.E2_TIPO" + cOperador + "SE2.E2_FORNECE" + cOperador + "SE2.E2_LOJA "
                Else
                    cUpdate5 += "AND SE2TIT.E2_TITPAI = CONCAT(SE2.E2_PREFIXO,SE2.E2_NUM,SE2.E2_PARCELA,SE2.E2_TIPO,SE2.E2_FORNECE,SE2.E2_LOJA) "                 
                EndIf
            ...
            cUpdate5 += "AND " + cTable + ".E5_FILIAL = '?2' "
            cUpdate5 += "AND " + cTable + ".SE5CARTEIR = '" + IIf(mv_par11 == 1, "R", "P") + "' "
            cUpdate5 += "AND " + cTable + ".SE5MANUAL = 0 "
            cUpdate5 += "AND " + cTable + ".SE5MAXSEQ = " + cTable + ".E5_SEQ "
            cUpdate5 += "AND " + cTable + ".D_E_L_E_T_ = ' ' "        
        EndIf        
    ...
    cDelete2 := "DELETE SE5TMP FROM " + cTable + " SE5TMP "
    
    If cSGBD $ "POSTGRES|ORACLE"
        cDelete2 := "DELETE FROM " + cTable + " SE5TMP "
    EndIf
    
    cDelete2 += "WHERE SE5TMP.E5_FILIAL = '?1' "
    cDelete2 += "AND SE5TMP.SE5CARTEIR = 'R' "
    cDelete2 += "AND SE5TMP.D_E_L_E_T_ = ' ' "
    cDelete2 += "AND EXISTS ("
    
    If mv_par11 == 1
        cDelete2 += "SELECT SE1.E1_FILIAL FROM " +  RetSQLName("SE1") + " SE1 "
        cDelete2 += "WHERE SE1.E1_FILIAL = '?2' "
        cDelete2 += "AND SE1.E1_PREFIXO = SE5TMP.E5_PREFIXO "
        cDelete2 += "AND SE1.E1_NUM = SE5TMP.E5_NUMERO "
        cDelete2 += "AND SE1.E1_TIPO = SE5TMP.E5_TIPO "
        cDelete2 += "AND SE1.E1_PARCELA = SE5TMP.E5_PARCELA "
        cDelete2 += "AND SE1.E1_CLIENTE = SE5TMP.E5_CLIFOR "
        cDelete2 += "AND SE1.E1_LOJA = SE5TMP.E5_LOJA "
        cDelete2 += "AND SE1.D_E_L_E_T_ = ' ' "
        cDelete2 += "AND SE1.E1_VENCREA NOT BETWEEN '" + DToS(MV_PAR31) + "' AND '" + DToS(MV_PAR32) + "')"
    Else
        cDelete2 += "SELECT SE2.E2_FILIAL FROM " +  RetSQLName("SE2") + " SE2 "
        cDelete2 += "WHERE SE2.E2_FILIAL = '?2' "
        cDelete2 += "AND SE2.E2_PREFIXO = SE5TMP.E5_PREFIXO "
        cDelete2 += "AND SE2.E2_NUM = SE5TMP.E5_NUMERO "
        cDelete2 += "AND SE2.E2_TIPO = SE5TMP.E5_TIPO "
        cDelete2 += "AND SE2.E2_PARCELA = SE5TMP.E5_PARCELA "
        cDelete2 += "AND SE2.E2_FORNECE = SE5TMP.E5_CLIFOR "
        cDelete2 += "AND SE2.E2_LOJA = SE5TMP.E5_LOJA "
        cDelete2 += "AND SE2.D_E_L_E_T_ = ' ' "
        cDelete2 += "AND SE2.E2_VENCREA NOT BETWEEN '" + DToS(MV_PAR31) + "' AND '" + DToS(MV_PAR32) + "')"    
    EndIf
    ...
    While SM0->(!EoF()) .And. SM0->M0_CODIGO == cEmpAnt .And.  If(MV_PAR40 == 1, (nSelFil < nLenSelFil) .And. cFilDe <= cFilAte, SM0->M0_CODFIL <= cFilAte)
        If MV_PAR40 == 1
            nSelFil++
        ...
        If lMultiNat
            Finr199(@nGerOrig, @nGerValor, @nGerDesc, @nGerJurMul, @nGerCM, @nGerAbLiq, @nGerAbImp, @nGerBaixado, @nGerMovFin, @nGerComp,;
                        ...
                        .F., cCondicao, cCond2, aColu, lContinua, cFilSE5, .T., Tamanho, @aRet, @aTotais, nOrdem, @nGerFat, @nFilFat, .T.)
        ...
        Else
            ...
            nSE5Recno := NEWSE5->(Recno())            
            cDlt2Fil  := StrTran(cDelete2, "?1", cXFilSE5)
            cDlt2Fil  := StrTran(cDlt2Fil, "?2", IIf(mv_par11 == 1, cXFilSE1, cXFilSE2))
            
            If TCSqlExec(cDlt2Fil) < 0
                DbOnError(TCSqlError(), "cDelete2")
            EndIf
            
            cUpd5Fil := StrTran(cUpdate5, "?1", IIf(mv_par11 == 1, cXFilSE1, cXFilSE2))
            cUpd5Fil := StrTran(cUpd5Fil, "?2", cXFilSE5)
            
            If TCSqlExec(cUpd5Fil) < 0
                DbOnError(TCSqlError(), "cUpdate5")
            EndIf
            
            //Posiciona no 1o registro válido para a filial corrente, caso não exista, vai para a próxima filial
            NEWSE5->(DbSeek(cXFilSE5))
            ...
            While NEWSE5->(!EoF()) .And. NEWSE5->E5_FILIAL == cXFilSE5
                ...
                While NEWSE5->(!EoF()) .And. NEWSE5->E5_FILIAL == cXFilSE5 .And. &cCond2 == cAnterior
                    lManual     := .F.
                    lSkpNewSe5  := .F.
                    cVerifAbat  := NEWSE5->SE5VERABAT
                    DbSelectArea("NEWSE5")

                    If NEWSE5->SE5MANUAL == 1
                        lManual := .T. 
                    EndIf
                    ...
                    While NEWSE5->(!EoF()) .And. NEWSE5->E5_FILIAL == cXFilSE5 .And.;
                        ...
                        If !Empty(NEWSE5->E5_NUMERO)
                            If (NEWSE5->E5_RECPAG == "R" .And. !(NEWSE5->E5_TIPO $ MVPAGANT + "/" + MV_CPNEG)) .Or. ;
                                ...
                                lAchou := SA1->(MsSeek(cXFilSA1 + NEWSE5->E5_CLIFOR + NEWSE5->E5_LOJA))
                                
                                If !lAchou
                                    cFilOrig := NEWSE5->E5_FILIAL //Procuro SA1 pela filial do movimento
                                    
                                    If MsSeek(cFilOrig + NEWSE5->E5_CLIFOR + NEWSE5->E5_LOJA)
                                        lAchou := Upper(AllTrim(SA1->A1_NREDUZ)) == Upper(AllTrim(NEWSE5->E5_BENEF))
                            ...
                            Else
                                DbSelectArea("SA2")
                                DbSetOrder(1)
                                lAchou := SA2->(MsSeek(cXFilSA2 + NEWSE5->E5_CLIFOR + NEWSE5->E5_LOJA))
                                
                                If !lAchou
                                    cFilOrig := NEWSE5->E5_FILIAL //Procuro SA2 pela filial do movimento
                                    
                                    If MsSeek(cFilOrig + NEWSE5->E5_CLIFOR + NEWSE5->E5_LOJA)
                                        lAchou := Upper(AllTrim(SA2->A2_NREDUZ)) == Upper(AllTrim(NEWSE5->E5_BENEF))
                        ...
                        If Empty((NEWSE5->(E5_PRETPIS + E5_PRETCOF + E5_PRETCSL)))
                            If nRecSE2 > 0 .And. lPccBaixa .And. cCarteira == "P"
                            ...
                            ElseIf lPccBxCr .And. cCarteira == "R" .And. !NEWSE5->E5_TIPO $ MVRECANT
                                nTotAbImp += (NEWSE5->(E5_VRETPIS + E5_VRETCOF + E5_VRETCSL + E5_VRETISS + E5_VRETINS))
                            EndIf
                        EndIf

                        If lAchou .And. !SE1->E1_TIPO $ MVRECANT + "|" + MV_CRNEG .And. Empty(NEWSE5->E5_PRETIRF)
                            lCalcIRBx  := (cPaisLoc == "BRA" .And. SA1->A1_RECIRRF == "1" .And. SA1->A1_IRBAX == "1")
                            If lCalcIRBx
                                nTotAbImp += NEWSE5->E5_VRETIRF 
                            EndIf                          
                        EndIf
                    ...
                    If !lManual
                        If cCarteira == "R"
                            ...
                            If cVerifAbat == "S"
                                nAbatImp   := 0
                                nAbat      := SumAbatRec(cPrefixo, cNumero, cParcela, SE1->E1_MOEDA, "V", dBaixa, @nAbatImp, , Nil, Nil, Nil, Nil, Nil, Nil, Nil, .T.)                                
                                nAbatLiq   := nAbat - nAbatImp
                                nTotAbImp  += nAbatImp
                            EndIf
                    ...
                    If ((nDesc + nValor + nJurMul + nVlMovFin + nAbatLiq) != 0) .Or. (NEWSE5->SE5VA != 0)
                        ...
                        If !lManual
                            ...
                            Else
                                ...
                                If cPaisLoc == "BRA"
                                    lCalcIRF:= Posicione("SA2", 1, cXFilSA2 + cCliFor190, "A2_CALCIRF") == "1" .Or.;
                        ...
                        aRet[nTamRet][ID_DESCONTO  ] := nDesc
                        aRet[nTamRet][ID_ABATIMENTO] := nAbatLiq
                        aRet[nTamRet][ID_IMPOSTO   ] := nTotAbImp

                        If nVlMovFin <> 0
                            aRet[nTamRet][ID_TOTALPAGO] := nVlMovFin
                        Else
                            aRet[nTamRet][ID_TOTALPAGO] := nValor
                        EndIf
                        ...
                        aRet[nTamRet][ID_TEMVALOR]  := aRet[nTamRet][ID_VALORPG] == 0 .Or. NEWSE5->SE5VA != 0
                        ...
                        If lOriginal .and. !Empty(cChaveTit) .And. aRet[nTamRet][ID_VALORORIG] != 0 .And. !oChaveTit[cChaveTit] == Nil
                            aRet[nTamRet][ID_LORIGINAL] := .F.
                        Else
                            oChaveTit[cChaveTit] := .T.
                        EndIf
                        ...
                        IF MV_PAR12 <> nMoedaBco
                            aRet[nTamRet][ID_VALORVA] := Round(xMoeda(NEWSE5->SE5VA,nMoedaBco,MV_PAR12,NEWSE5->E5_DATA,nDecs+1,nTaxa),nDecs+1)
                        ENDIF

                        If !lRelMulNat
                            SE5->(MsGoTo(aRet[nTamRet][ID_E5RECNO]))
                            If !lRelatInit
                                lRelatInit := .T.
        ...
        If TCSqlExec("DELETE FROM " + cTable + " WHERE D_E_L_E_T_ = ' ' AND E5_FILIAL = '" + cXFilSE5 + "'") < 0
            DbOnError(TCSqlError(), "cDelete3")
        EndIf
...
/*/
Function U_F190MovBco(cMotBx As Character) As Logical
...
/*/
Function U_F190Fields() As Array
...
/*/{Protheus.doc} FR190Log
Função para logar os erros de execução do TcSQLExec.

@type       function
@author     Rafael Riego
@since      27/04/20
@obs        cópia da function FR130Log
/*/
...
/*/{Protheus.doc} DeleteChq
Efetua a montagem do comando SQL de exclusão dos registros.

@type       function
@author     Rafael Riego
@since      27/04/20
@param      cSGBD, characater, SGBD utilizado para montagem da query
/*/
...
/*/{Protheus.doc} DbOnError
Rotina acionada após encontrar error nos comandos SQL executados.

@type       function
@author     Rafael Riego
@since      25/09/2020
@param      cSQLError, character, erro ocorrido no banco de dados
@param      cVariable, character, variável usada para guardar o comando SQL
@return     Nil
/*/
...
@author     Rafael Riego
@since      08/10/2020
@param      aMovimento, array, movimento atual que terá seus valores zerados por ser VA
@return     Nil
/*/
