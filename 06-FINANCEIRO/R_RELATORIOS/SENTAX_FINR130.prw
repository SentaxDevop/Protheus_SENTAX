...
±±≥FunáÖo    ≥ FINR130  ≥ Autor ≥ Daniel Tadashi Batori ≥ Data ≥ 01.08.06 ≥±±
...
Static Function ReportDef()
    ...
    __lProcSaldoTit	:= ExistProc( IIf( FindFunction( "GetSPName" ), GetSPName( "FIN002", "10" ), "FIN002" ), '015')//STATICCALL(FINXFIN,VERIDPROC) )

    oReport := TReport():New("FINR130",STR0005,"FIN130",{|oReport| ReportPrint(oReport)},STR0001+STR0002)

    //oReport:DisableOrientation(.T.)// OpÁ„o de impress„o no formato Retrato desabilitada devido a n„o comportar mais informaÁıes.
    ...
    */
    ...
    //≥ mv_par20		 // Compıe Saldo Retroativo?							≥
...
Static Function ReportPrint(oReport)
    ...
    Local lExistAba		:= .F. // Verifica se existem tÌtulos de abatimento
    Local lVl_Corr		:= .T.
    ...
    Local aDtTit		:= {}
    Local nTxZero       := 0
    Local lIsTxContr    := .F.      // Define se È taxa contratada
    Local nTotAtVenc    := 0
    Local nTotVlOri     := 0
    Local nTotEmDia     := 0
    Local nVlrTotJur    := 0
    ...
    //Efetua a separaÁ„o das filiais do grupo de empresa atual e ordena por cÛdigo de filial
    ...
    ElseIf Empty(aSelFil)
        If lConFilAbx
            //Se filial diferente de branco e filial existente busca posiÁ„o inicial
            If !(Empty(MV_PAR22))
                If FwFilExist(cEmpAnt, MV_PAR22)
                    ...
                    EndIf
                Else
                    // caso informado uma cÛdigo de filial inexistente, seta onde deve iniciar a iteraÁ„o, caso n„o, seta como 1
    ...
    //Compartilhamento
    //N„o usa controle por empresa
    If (nTamEmp := Len(FwSM0Layout(, 1))) == 0
        cMAEmpSA1   := cMAFilSA1
        cMAEmpSE1   := cMAFilSE1
    EndIf

    //N„o usa controle por unidade de negÛcio
    ...
    */
    If mv_par19 == 1 //1 = Analitico - 2 = Sintetico
        ...
        oReport:SetTotalText({|| STR0038 + "(" + AllTrim(Str(nTotTit))+" "+If(nTotTit > 1, STR0039, STR0040)+")" + If(nTxZero > 0,STR0087, " ")}) //"  ****** SALDO INCORRETO, POIS EXISTEM TÕTULOS EM MOEDAS ESTRANGEIRAS CONVERTIDAS COM TAXA DE MOEDA ZERO. VERIFIQUE QUAIS OS TÕTULOS COM A DESCRI«√O 'SEM TAXA' NA COLUNA HIST”RICO"
    ...
    /* Cria tempor·rio inicial para processamento.
    Insert Inicio */
    ...
    Elseif nOrdem = 5
        ...
        cOrder := SqlOrder("E1_FILIAL+E1_NATUREZ+E1_NOMCLI+E1_PREFIXO+E1_NUM+E1_TIPO+E1_FILORIG") // Ser· usado para FINR135
    ...
    // Salvo dados do Where para posterior utilizaÁ„o
    If cMAFilSE1 == "E"
    ...
    /* Retira os titulos que n„o compoem o fluxo de caixa */
    ...
    // CabeÁalho principal + Where Principal
    ...
    // Caso despreze os valores n„o adiciona os itens e mante os valores normais.
    If MV_PAR33 != 3
        ...
        /* Vincula apenas aos registros que est„o na query principal */
    ...
    If MV_PAR37 == 1
        ...
        EndIf
    Else
    // Atualia os campos de data do movimento para posterior deleÁ„o
        If lPostgre
    ...
    If lMstErro .And. !cSGBD $ "ORACLE#DB2#INFORMIX#MSSQL#POSTGRES#OPENEDGE#CTREESQL" // Alerta o usu·rio que o
        //"Ocorreram inconsistÍncias na utilizaÁ„o de comandos no banco de dados! Banco de dados n„o homologado"
        HELP(" ",1,"SGDBInfo",,STR0091 ,2,0,,,,,, { STR0092 })
        //"Avalie o log de incosistÍncias gerado na pasta system."
    EndIf
    ...
    If MV_MULNATR .And. nOrdem == 5 
        ...
        While lContinua
            ...
            //Bloco utilizado para nao duplicar registros quando Filial compartilhada na gest„o corporativa. Exempo SE1:
            ...
            // Fazer query no tempor·rio para repassar ao FINR135
                ...
                While !Eof() .And. SE1->E1_FILIAL == cFilSE1 .And. &cCond1
                    ...
                    If SE1->E1_FILIAL == cFilSE1 .And. &cCond2 == cCarAnt                        
                        While !Eof() .And. SE1->E1_FILIAL == cFilSE1 .And. &cCond2 == cCarAnt                            
                            ...
                            /*
                            Modificado tratamento para o par‚metro MV_PAR36(CONSIDERA DATABASE) para quando se imprime um relatÛrio com o par‚metro MV_PAR20(SALDO
                            RETROATIVO) = SIM verificando corretamente na SE5 a data de baixa, crÈdito ou digitaÁ„o para impress„o.
                            */
                            ...
                            // Verifica se trata-se de abatimento ou somente titulos atÈ a data base.
                            If (SE1->E1_TIPO $ MVABATIM	+"/"+MVFUABT .And. mv_par33 != 1) .Or. (SE1->E1_EMISSAO > mv_par36 .and. MV_PAR38 == 2)
                                cMTitPai := Iif(!Empty(SE1->E1_TITPAI), SE1->E1_TITPAI, U_FTITPAI())
                            ...
                            If lSldRetro	// Saldo Retroativo = Sim
                                ...
                                //Verifica se existem compensaÁıes em outras filiais para descontar do saldo, pois a SaldoTit() somente verifica as movimentaÁıes da filial corrente.
                                If __lProcSaldoTit  .or. (!(AllTrim(SE1->E1_TIPO) $ MVRECANT + "|" + MV_CRNEG) .and. !__lProcSaldoTit .and. __cMVFinFix < "20" )
                                    If lVerCmpFil .and. SE1->E1TEMCMP = 'S'
                                        ...
                                        proclogatu("FIM",STR0085)//Log pesquisa de compensaÁ„o
                                    EndIf
                                EndIf
                                
                                If !Empty(dUltBaixa := SE1->E1_BAIXA) .And. dDataBase < dUltBaixa                                    
                                    dUltBaixa := FR130DBX() // Ultima baixa atÈ DataBase
                                EndIf
                                ...
                                //certifica-se de que est· posicionado no cliente correto, para que seja deduzido corretamente o IR baixado do saldo do tÌtulo.
                                ...
                                If cPaisLoc == "BRA"
                                    If (( cMVBR10925 == "1" .and. SE1->E1_EMISSAO <= MV_PAR36 .and. !(SE1->E1_TIPO $ "PIS/COF/CSL").and. !(SE1->E1_TIPO $ MVABATIM) ) ;
                                        ...
                                        //tratamento para emiss„o correta do saldo do tÌtulo, quando o PCC+IR estiver na baixa e for realizado baixa parcial.
                                        If nRecSA1 > 0
                                            SE5->(DbSetOrder(7))
                            ...
                            If ! SE1->E1_TIPO $ MVABATIM	+"/"+MVFUABT
                                If ! (SE1->E1_TIPO $ MVRECANT+"/"+MV_CRNEG) .And. ;
                                    ...
                                    //Busca as informaÁıes para alimentar a array aTitImp utilizando a funÁ„o F130RETIMP
                                    If lExistAba
                                        If __aTtImp == Nil
                                            __aTtImp := {}
                                        EndIf
                                        If ( nPos := (aScan(__aTtImp, {|x| x[1] == cFilNat}))) == 0
                                            aTitImp		:= U_F130RETIMP(cFilNat)
                            ...
                            If lSldRetro  //Recompoe Saldo Retroativo
                                ...
                                IF !Empty(SE1->E1_BAIXA)
                                    ...
                                    EndIf
                                Else
                                    //Titulo n„o foi Baixado e foi transferido para Carteira e Data Movimento e menor ou igual a Data Base do RelatÛrio
                            ...
                            If SE1->E1_TIPO $ MVABATIM +"/"+MVFUABT // Caso for tÌtulo de abatimento, busca o vencimento do tÌtulo pai
                            ...
                            If dDataBase > dVencRea	//vencidos
                                If mv_par19 == 1 //1 = Analitico - 2 = Sintetico
                                    ...
                                    Endif
                                EndIf
                                // Somente chamad fa070juros se realmente houver necessidade de calculo de juros ou o c·lculo da multa È feito pelo SIGALOJA
                                If !( SE1->E1_TIPO $ MVRECANT + "|" + MV_CRNEG ) .And. ( lFJurCst .Or. !Empty(SE1->E1_VALJUR) .Or. !Empty(SE1->E1_PORCJUR) .Or. lClcMultLj )
                                    nBaseJuros := 0

                                    //Caso n„o possua nenhuma baixa - utiliza SE1->E1_SALDO
                                    If Empty(SE1->E1_BAIXA)
                                        nBaseJuros := SE1->E1_SALDO
                                    //Caso possua baixa, porÈm nenhuma baixa tenha sido encontrada atÈ a database informada - utiliza SE1->E1_VALOR
                                    ...
                                    ElseIf AllTrim(SE1->E1_ORIGEM) == "FINI055" //IntegraÁ„o TIN x PROTHEUS
                                        If lMvJurTin
                            ...
                            If mv_par19 == 1 //1 = Analitico - 2 = Sintetico 18
                                ...
                                // pois se cliente customizar relatorio com campo memo, o conteudo n„o È exibido
                        ...
                        Enddo
                    Else
                        // ProteÁ„o para n„o entrar em loop
                        SE1->(DbSkip())
                        Loop
                    EndIf
                    __oHash:Clean()
                    ...
                    IF nTit5 > 0 .And. nOrdem != 2 .And. nOrdem != 10 .And. mv_par19 == 2 //1 = Analitico - 2 = Sintetico
                        U_SubTot130R(nTit0,nTit1,nTit2,nTit3,nTit4,nOrdem,cCarAnt,nTotJur,nDecs,oReport,,oSection2)
                        nTotReg++
                    ElseIf lImpSintTbl
                        oReport:SkipLine()
                        oReport:ThinLine()
                    ...
                    //MV_PAR33 = 2 (N„o Listar) os tÌtulos de abatimento n„o ser„o impressos,porÈm do valor a receber ser„o diminuidos os abatimentos.
    ...
    If __lMetric .and. nTotTit > 0
        nTimer := oTimer:ElapseTime(.T.)//o true vai fazer reiniciar, caso vc queira usar de novo o oTimer...aÌ ele comeÁa a contar de 
        nMedTimer := nTimer/nTotTit
        If lSldRetro
            FwCustomMetrics():setAverageMetric("SldRetroSIM", "financeiro-protheus_tempo-conclus„o-processo_seconds", nMedTimer)
        Else
            FwCustomMetrics():setAverageMetric("SldRetroNAO", "financeiro-protheus_tempo-conclus„o-processo_seconds", nMedTimer)
        EndIf
    EndIf
...
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
/*/
User Function SubTot130R(nTit0,nTit1,nTit2,nTit3,nTit4,nOrdem,cCarAnt,nTotJur,nDecs,oReport,aDados,oSection)
...
‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥ SumBxPCC≥ Autor ≥ Igor Franzoi			 ≥ Data≥19/12/2011 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Soma os abatimentos do PCC em caso de saldo retroativo	   ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥Uso       ≥ Finr130.prx                                                 ≥±±
...
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
*/
User Function F130RETIMP(cFiltro)
    Local aTitulos := {}
...
±±∫          ≥ branco a funÁao ira preencher para o relatorio ficar correto±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Uso       ≥ FINR130                                                   ∫±±
±±»ÕÕÕÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕº±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
cQuery += " AND E1_TIPO NOT IN " + U_F130MontaIn()
...
Local nTamTipo := 0
Local cTipo    := ""
//Controla o Pis Cofins e Csll na RA (1 = Controla retenÁ„o de impostos no RA; ou 2 = N„o controla retenÁ„o de impostos no RA(default))
...
/*/{Protheus.doc} FR130DBX
    Busca a data da ultima baixa realizada do titulo a receber atÈ a DataBase do sistema.

    @author leonardo.casilva

    @since      11/04/2014
    @version    P1180
    @param cChaveTit, Char, Chave do tÌtulo com os campos separados por |
    @Param cCampo, Char, Campo de data que ser· retornado
    @return     dDataRet
/*/
...
If !lTemCli
    ...
    If __oTCli == Nil
        ...
        cQuery += "    AND SE1.E1_TIPO IN "  + U_F130MontaIn(@cTipoIn)
...
/*/
//-------------------------------------------------------------------
User Function F130MontaIn(cTipoIn)
...
FunÁ„o para transformar o ano em 2 caracteres.

@author Douglas GonÁalves de Souza
@since 16/05/17
@version 12.1.14

@return Retorna um array com as datas do tÌtulo transformadas
    aDtsTit	:= {E1_EMISSAO, E1_VENCTO, E1_VENCREA}
/*/
//-------------------------------------------------------------------
User Function F130Dtano2(cAliasSE1 As Character) As Array
...
FunÁ„o para Executar o ChangeQuery no where dos updates

@author Fernando Navarro
@since 23/04/20

@return Retorna o novo where j· ajustado conforme o banco de dados.
/*/
//-------------------------------------------------------------------
...
FunÁ„o para logar os erros de execuÁ„o do TcSQLExec.
...
If lContinua
    ...
    cThreadID 	:= AllTrim(Str(ThreadID())) 	//Retorna o ID (n˙mero de identificaÁ„o) da thread em que a chamada da funÁ„o foi realizada
    cProcLine 	:= AllTrim(Str(ProcLine(1))) 	//Retorna o n˙mero da linha do cÛdigo fonte executado que fez a chamada da geraÁ„o do LOG
    cFunction 	:= ProcName(1) 					//Retorna o nome da funcao em execuÁ„o que fez a chamada da geraÁ„o do LOG
...
@since      05/04/2024
@param      cFilSE5, character, filial do tÌtulo na tabela SE5
@param      cPrefSE5, character, prefixo do tÌtulo na tabela SE5
@param      cNumSE5, character, numero do tÌtulo na tabela SE5
@param      cParcSE5, character, parcela do tÌtulo na tabela SE5
@param      cTpSE5, character, Tipo do tÌtulo na tabela SE5
@param      cCliSE5, character, Cliente do tÌtulo na tabela SE5
...
Verificar existÍncia da FKD para a filial solicitada.
...
Realiza busca por tÌtulos que foram compensados na tabela SE5 e retorna seu valor.
