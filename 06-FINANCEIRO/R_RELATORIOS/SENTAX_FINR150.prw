...
Static Function ReportPrint(oReport)
	...
	lLandsCape := Iif(oReport:GetOrientation() == 1 ,.T.,.F.)

	If Type("aFiltro") == "A" .And. Len(aFiltro) > 0
        oSection1:AuserFilter := aFiltro
    EndIf
	...
	ElseIf !lSelectFil .And. lConFilAbx
		If Empty(MV_PAR23) .And. Empty(MV_PAR24)
			Help(" ", 1, "EMPTYFIL",, STR0073, 1,0,,,,,, {STR0074}) //"Filial inicial e filial final não informados" "Informe ao menos a filial final para emitir o relatório"
			lCancPrint := .T.
		ElseIf MV_PAR24 < MV_PAR23
			Help(" ", 1, "PARAMFIL",, STR0075, 1,0,,,,,, {STR0076}) //"Filial final não pode ser maior que a filial final" //"Informe uma filial final maior que a filial inicial para emitir o relatório"
			lCancPrint := .T.
		EndIf
	EndIf
	...
	If lLandsCape
		oSection1:Cell("TITULO"):SetSize(25)
		oSection1:Cell("E2_NATUREZ"):SetSize(20)	
		oSection1:Cell("E2_EMISSAO"):SetSize(25)
		oSection1:Cell("E2_VENCTO"):SetSize(25)
		oSection1:Cell("E2_VENCREA"):SetSize(25)
	EndIf
	...
	If MV_PAR20 == 1	//1- Analitico  2-Sintetico
		...
		TRFunction():New(oSection1:Cell("E2_HIST"), "", "ONPRINT", oBreak,, IIf(cPaisLoc == "CHI", cPictTit, PesqPict("SE2","E2_VALOR")), {|lSection,lReport| If(lReport,nGerTot,nTotVenc)}, .F., .T.)
	EndIf
	...
	While nInc <= nLenSelFil
		If lContinua
			...
			If lSelectFil
				cQueryP += " AND "+ FinSelFil(aSelfil1,"SE2")
			ElseIf lConFilAbx // Considerar Filiais a baixo ? = Sim
			...
			If TCSqlExec(cQuery) < 0
				FR150Log(STR0068 + TCSQLError() , "Query1")//"Ocorreu um problema na execução do comando SQL no seu banco de dados, avalie os detalhes:"
				lMstErro := .T.
			EndIf
			...
			If lExistFJU .And. MV_PAR38 == 1
				...
				If TCSqlExec(cQuery) < 0
					FR150Log(STR0068 + TCSQLError() , "Query2")//"Ocorreu um problema na execução do comando SQL no seu banco de dados, avalie os detalhes:"
					lMstErro := .T.
				EndIf
			EndIf
			...
			If lMstErro .And. !(__cSGBD $ "ORACLE#DB2#INFORMIX#MSSQL#POSTGRES#OPENEDGE#CTREESQL")
				...
				HELP(" ",1,"SGDBInfo",,STR0070 ,2,0,,,,,, { STR0071 }) //"Ocorreram inconsistências na utilização de comandos no banco de dados! Banco de dados não homologado!"#"Avalie o log de incosistências gerado na pasta system."
			EndIf
			...
			While &cCond1 .And. SE2->(!EoF()) .And. lContinua .And. SE2->E2_FILIAL == xFilial("SE2")
				...
				While &cCond2 == cCarAnt .And. SE2->(!EoF()) .And. lContinua .And. SE2->E2_FILIAL == xFilial("SE2")
					...
					Else
						If cPaisLoc <> "BRA"
							nValtit := xMoeda(SE2->E2_VALOR , SE2->E2_MOEDA  , MV_PAR15, SE2->E2_EMISSAO , ndecs+1 , If(MV_PAR35 == 1 , SE2->E2_TXMOEDA , If(lSemTaxaM2 , 1 , nTaxaDia)),  If(SE2->E2_TXMOEDA <> 0 .AND. se2->e2_moeda>1, 0, RecMoeda(dDataReaj , MV_PAR15))) * If(SE2->E2_TIPO $ MV_CPNEG + "/" + MVPAGANT , -1 , 1)
							aDados[VL_ORIG]	:= round(nValtit,MsDecimais(SE2->E2_MOEDA))
						Else	
				...
				If MV_PAR20 == 2	//1- Analitico  2-Sintetico
					...
					If nTit5 > 0 .And. nOrdem != 1
						U_SubT150R(nTit0, nTit1, nTit2, nTit3, nTit4, nOrdem, cCarAnt, nTotJur, oReport, oSection1, nRecSE2Tmp)
					EndIf
...
@since 01/06/92
/*/
User Function SubT150R(nTit0, nTit1, nTit2, nTit3, nTit4, nOrdem, cCarAnt, nTotJur, oReport, oSection1, nRegSE2)
	Local cQuebra := ""
	Local aAreaSE2:= {}
...
Static Function ImpT150R(nTot0,nTot1,nTot2,nTot3,nTot4,nTotTit,nTotJ,nTotTit,oReport,oSection1)
	...
	oSection1:Cell("FORNECEDOR"):SetBlock({|| STR0027 + "(" + AllTrim(Str(nTotTit))+" "+ Iif(nTotTit > 1,STR0028,STR0029)+")" })
...
Static Function FR150Log(cLogText As Character, cSQLControl As Character)
	...
	If !File(cLogFile)
		...
		If nHandle == -1
			lContinua := .F.
		Else
			cLogHead:= STR0069  + DToC(date()) + CRLF//"Log de incosistências de execução dos comandos SQL do FINR150 em "
...
Static Function VerifProc()
	...
	If __lPFIN002 == Nil
		...
		__lPFIN002	:= .T.//ExistProc(cProcedure, StaticCall(FinXFin, VerIdProc))
	EndIf
	...
	If !(IsBlind())
		...
		If __lPFIN002 .And. (!__lCmpoFK2 .Or. !__lCmpoFK1)
			MsgAlert(STR0077, STR0078) //"Execute o UPDDISTR de acordo com a última expedição contínua para criação dos dicionários necessários para a execução integral de todos os recursos do relatório." "Atenção"
		EndIf
	EndIf
...
@return		cTmpAlias, character, alias da tabela temporária
/*/
Static Function CleanTable(cTableName As Character, cTmpAlias As Character )
	...
	If !Empty(cTableName)
		nOrder := (cTmpAlias)->(IndexOrd())

		TcSqlExec("DELETE FROM "+ cTableName)

    	(cTmpAlias)->(dbSetOrder(0))         //- mudar para a ordem de recno
    	(cTmpAlias)->(DbSetFilter( { || .T.  }, "1=1" ) )
    	(cTmpAlias)->(DbClearFilter())
    	(cTmpAlias)->(dbGotop())             //- envio para o topo da tabela
    	(cTmpAlias)->(dbSetOrder(nOrder))    //- retorna a ordem salva
    EndIf
