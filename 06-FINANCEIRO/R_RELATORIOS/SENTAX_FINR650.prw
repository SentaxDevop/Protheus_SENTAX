...
Impressão do Retorno da Comunicação Bancária  
@type function
@version 12
@author totvs
@since 03/02/2023
...
/*/{Protheus.doc} Function ReportPrint
	...
	relatorios que poderao ser agendados pelo usuario.
	@params  oReport = Objeto Report do Relatório
...
Static Function ReportPrint(oReport as Object) as Logical
	...
	While (nTamArq-nLidos) >= nTamDet
		...
		If mv_par08 != 3
			...
			EndIf
			If !lDDA .And. !lAchouSEB //A inclusão do 2º dbSeek é necessário porque o preenchimento do campo EB_MOTBAN não é obrigatório
		...
		If lOk
			...
			If lOk
				...
				Else
					If mv_par07 == 1
						cDescr2 := OemToAnsi(STR0022)  	//TÍTULO OK
					Else
