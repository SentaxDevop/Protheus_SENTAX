...
User Function XmlNfeSef(cTipo,cSerie,cNota,cClieFor,cLoja,cNotaOri,cSerieOri)
	...
	Local cAliasSD1  	:= "SD1"
	Local cAliasSD2  	:= "SD2"
	local cAliasSB5   	:="SB5" //Customização
	...
	Local cCFOPExp 		:= "1501-2501-5501-5502-5504-5505-6501-6502-6504-6505-3102-3101-3949" //customização


	//-------------------------------------------------------------------------------------------------------
	// TOTVS Unidade Curitiba //customização
	//-------------------------------------------------------------------------------------------------------
	Local _cMensCli  := ""
	Local  _cOrdem   := '% D2_FILIAL,D2_DOC,D2_SERIE,D2_CLIENTE,D2_LOJA,D2_ITEM,D2_COD %'
	//-------------------------------------------------------------------------------------------------------
	...
	Local nCrdPres		:= 0
	Local nTotCrdP      := 0
	Local lDKD			:= AliasInDic("DKD") .and. DKD->(ColumnPos("DKD_VALBRU")) > 0
	Local aRetPgLoj 	:= {}
	Local aProcRef		:= {}
	...
	If cTipo == "1"
		...
		For nCountNF := 1 To Len(aCupRefLoj)
			...
			If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
				...
				//---------------------------------------------------------------------		
				// AFSouza - customização		
				// Muda a ordem dos items
				//---------------------------------------------------------------------		
				_cOrdem := '% D2_FILIAL,D2_DOC,D2_SERIE,D2_CLIENTE,D2_LOJA,D2_ITEM,D2_COD %'
				//------
				...
				Else
					...
					While ( !Empty(cScan) )
						...
						If !empty(cEspecie)
							...
							// Grava Log com relação ao volume - 28/09/2016	alterado AFSOUZA //customização	
							U_AFAT004( SF2->F2_FILIAL, SF2->F2_DOC, SF2->F2_SERIE, SF2->F2_EMISSAO, SF2->F2_VOLUME1, SF2->F2_VOLUME1, "NFESEFAZ", dDataBase, Time(), UsrFullName(RetCodUsr()), "Transmissão da nota. Preparando o XML." )				
					...
					If !Empty(SF2->F2_DUPL)	.Or. lVLojaDir
                        ...
                        //AFSOUZA - Trocado campo E1_VENCORI por E1_VENCREA - customização
						If lLJPRFPad
							...
							BeginSql Alias cAliasSE1
								...
								SELECT E1_FILIAL,E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_VENCREA,E1_VALOR,E1_VLCRUZ,E1_ORIGEM,E1_PIS,E1_COFINS,E1_CSLL,E1_INSS,E1_VLRREAL,E1_IRRF,E1_ISS,E1_NATUREZ,E1_EMISSAO
							FROM %Table:SE1% SE1
							WHERE
						...
						While (cAliasSE1)->(!Eof()) .And. ( ( lLJPRFPad .AND. (cAliasSE1)->E1_FILIAL+(cAliasSE1)->E1_PREFIXO+(cAliasSE1)->E1_NUM == cChaveSE1 ) .OR.;
							...
							If (cAliasSE1)->E1_TIPO = MVNOTAFIS .OR. (cAliasSE1)->E1_TIPO = 'DP' .OR. ((Alltrim((cAliasSE1)->E1_ORIGEM) $ 'LOJA701|FATA701|LOJA010') .AND. (cAliasSE1)->E1_TIPO $ cWhere)
								...
								If cValLiqB == "2" .Or. (lMV_NFSEPCC .and. cMVEstado == "DF" ) //1-Baixa / 2-Emissao
									aadd(aDupl,{(cAliasSE1)->E1_PREFIXO+(cAliasSE1)->E1_NUM+(cAliasSE1)->E1_PARCELA,(cAliasSE1)->E1_VENCREA;//AFSOUZA - Trocado campo E1_VENCORI por E1_VENCREA - customização
								...
								Else
									aadd(aDupl,{(cAliasSE1)->E1_PREFIXO+(cAliasSE1)->E1_NUM+(cAliasSE1)->E1_PARCELA,(cAliasSE1)->E1_VENCREA,nValDupl}) //AFSOUZA - Trocado campo E1_VENCORI por E1_VENCREA - customização
								EndIf
							EndIf
						...
						If lContinua
							...
							Else
								...
								*/
								...
								//------------------------------------------------------------------------------------------------------
								//AFSOUZA - Código do cliente e descrição nas informações adicionais do produto (tag infAdProd)	
								//customização				
								//-----------------------------------------------------------------------------------------------------
								...
								IF Alltrim((cAliasSD2)->(D2_CLIENTE)) == "8217"
									If SA7->(MsSeek( xFilial("SA7") + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD) )) .AND. ;						
										!Empty(SA7->A7_CODCLI)
										...
										SC6->C6_NUMPCOM
										cInfAdic += "IAP01"+SA7->A7_CODCLI+space(18)+STRZERO(VAL(SC6->C6_NUMPCOM),12)
								...
								Else
									...
									If SA7->(MsSeek( xFilial("SA7") + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD) )) .AND. ;						
										!Empty(SA7->A7_CODCLI) .AND. ;
										!Empty(SA7->A7_DESCCLI) 				                    
							
										cInfAdic += "Cod. Cliente: " + AllTrim(SA7->A7_CODCLI) + " - Desc: " + SA7->A7_DESCCLI
								...
								If MsSeek(xFilial("SB5")+(cAliasSD2)->D2_COD)
									If SB5->(FieldPos("B5_DESCNFE")) > 0 .And. !Empty(SB5->B5_DESCNFE)
										cInfAdic += Alltrim(SB5->B5_DESCNFE) //AFSOUZA - Alterado de := para += //customização
									Else
										cInfAdic += "" //AFSOUZA - Alterado de := para += //customização
									EndIf

									//customização
									If SB1->B1_XRISCO == "S" // IDENTIFICA SE EXISTE PRODUTO DE RISCO    
										_lPrdRisco := .T.
										U_AEST001C( SB5->B5_COD, SB5->B5_CEME, SB5->B5_XNONU, SB5->B5_XCLASRI, SB5->B5_XSUBCLA, SB5->B5_XGRPEMB, SB5->B5_XRISSUB, @cInfAdic )							
										cMensONU	:= " - DECLARAMOS QUE OS PROTUDOS ESTAO ADEQUADAMENTE EMBALADOS E/OU ACONDICIONADOS "
										cMensONU	+= "PARA SUPORTAR OS RISCOS NORMAIS DE CARREGAMENTO, DESCARREGAMENTO E TRANSPORTE "
										cMensONU	+= "CONFORME LEGISLACAO VIGENTE.  EMERGENCIA 24 HORAS - PRODUTOS QUIMICOS/PRODUTOS PERIGOSOS, "
										cMensONU	+= "CONTATAR DDG 33608500 - "
									EndIF
	...
	Else
		...
		If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
			...
			// Muda a ordem dos items                               
			//---------------------------------------------------------------------		
			_cOrdem := '% D1_FILIAL,D1_DOC,D1_SERIE,D1_FORNECE,D1_LOJA,D1_ITEM,D1_COD %'
			//---------------------------------------------------------------------		
			...
			Else
				...
				If !Empty(SF1->F1_DUPL)	
					...
					//AFSOUZA - Trocado campo E2_VENCORI por E2_VENCREA //customização
					BeginSql Alias cAliasSE2
						COLUMN E2_VENCREA AS DATE
						SELECT E2_FILIAL,E2_PREFIXO,E2_NUM,E2_PARCELA,E2_TIPO,E2_FORNECE,E2_LOJA,E2_VALOR,E2_VLCRUZ,E2_ORIGEM,E2_ISS,E2_VENCORI,E2_VENCREA
						FROM %Table:SE2% SE2
						WHERE
					...
					While !Eof() .And. xFilial("SE2") == (cAliasSE2)->E2_FILIAL .And.;
						...
						aadd(aDupl,{(cAliasSE2)->E2_PREFIXO+(cAliasSE2)->E2_NUM+(cAliasSE2)->E2_PARCELA,(cAliasSE2)->E2_VENCREA,(cAliasSE2)->E2_VLCRUZ - IIF(!lRuleDescISS, (cAliasSE2)->E2_ISS, 0) }) //Trocado campo E2_VENCORI por E2_VENCREA
						EndIf
				...
				BeginSql Alias cAliasSD1			
						...
						SD1.D1_FORMUL  = 'S' AND
						SD1.%NotDel%
						ORDER BY %Exp:_cOrdem%  //TOTVS Curitiba //customização
				EndSql
	...
	If !Empty(aNota)
		...
		//---------------------------------------------------------------------		
		//TOTVS Unidade Curitiba
		//customização
		//---------------------------------------------------------------------		
		If !Empty(_cMensCli)
			cMensCli := _cMensCli + " " + cMensCli
		Endif
		//---------------------------------------------------------------------		
...
Static Function NfeIde(cChave,aNota,cNatOper,aDupl,aNfVinc,cVerAmb,aNfVincRur,aRefECF,cIndPres,aDest,aProd,aExp,aComb,cIndIntermed,lChvCdd,aNfVCdd,lExpCDL,cCST,cModalid,oNfTciIntg)
	...
	if lGerChv
		cChave := SpedGerChv(aNota,aUF,cNumNf,allTrim(cModalid))
...
/*/
//-----------------------------------------------------------------------
static function GetTagRef(aNotaAjuste, cItemDoc)
	Local cStrRet   := ''
	Local cAliasRef := ''

	if lExisteDkn
		cAliasRef := GetDocRef(aNotaAjuste, cItemDoc)
		if (cAliasRef)->(!eof())
...
@param cItem, Character, Item do documento de ajuste
@return cAliasQry, Character, Alias da consulta realizada
/*/
Static Function GetDocRef(aDocAjuste, cItem)
	Local cQuery 	:= ''
	Local cAliasQry := ''
	Local cTipoMov 	:= '2'
	...
	cQuery += " 	AND DKN_LOJA   = ? " 
	if !empty(cItem) 
		cQuery += " 	AND DKN_ITEMNF = ? "
	endif
	cQuery := ChangeQuery(cQuery)

	oQueryDKN:SetQuery(cQuery)
	oQueryDKN:SetString(++nSeqField, ' ')
	oQueryDKN:SetString(++nSeqField, xFilial('DKN'))
	oQueryDKN:SetString(++nSeqField, cTipoMov)
	oQueryDKN:SetString(++nSeqField, aDocAjuste[2])
	oQueryDKN:SetString(++nSeqField, aDocAjuste[1])
	oQueryDKN:SetString(++nSeqField, aDocAjuste[7])
	oQueryDKN:SetString(++nSeqField, aDocAjuste[8])
	if !empty(cItem) 
		oQueryDKN:SetString(++nSeqField, cItem)
	endif	
...
/*/{Protheus.doc}  Function setOtherInfo
	...
	@param cNome		Nome do atributo do Json
	@param cConteudo	Conteudo dos cadastros.
	...
	@return string
/*///--------------------------------------------------------------------
Static function setOtherInfo(oItensReforma ,cNome, xConteudo)

	default cNome     		:= ""
	default xConteudo 		:= nil
	default oItensReforma 	:= nil
	
	if oItensReforma <> nil .and. !Empty(cNome)
		oItensReforma[cNome] := xConteudo
	endif
